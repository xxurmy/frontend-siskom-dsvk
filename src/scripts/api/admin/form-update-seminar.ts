// src/scripts/form-update-seminar.ts
...
interface UpdateSeminarResponse {
  message: string;
  seminar: Seminar;
}
...
const SYARAT_LIST: SyaratDefinition[] = [
  ...
  { key: "kartu_seminar", label: "Kartu Seminar (min. 10x)" },
  { key: "makalah", label: "Makalah Seminar" },
];
...
// State halaman
// ------------------------------------------------------------------
let seminarId: number | null = null;
...
// ------------------------------------------------------------------
// Warna badge <select> Status Seminar
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
function getSeminarIdFromUrl(): number | null {
...
function disableForm(): void {
  const form = document.getElementById("form-update-seminar") as HTMLFormElement | null;
...
// ------------------------------------------------------------------
// Muat detail seminar -> isi semua field form
// ------------------------------------------------------------------
async function loadSeminar(id: number): Promise<void> {
  const json = await apiFetch<Seminar>(`/auth/seminar/${id}`);
  if (!json) return;
...
async function handleSubmit(e: SubmitEvent): Promise<void> {
  ...
  try {
    const json = await apiFetch<UpdateSeminarResponse>(`/auth/seminar/${seminarId}`, {
      method: "PATCH",
...
async function fetchSyaratAdministrasi(id: number): Promise<void> {
  try {
    const json = await apiFetch<SyaratAdministrasiResponse>(
      `/auth/seminar/${id}/syarat-administrasi`
    );
    syaratData = json;
  } catch (err) {
...
async function openBerkasModal(): Promise<void> {
  ...
  await fetchSyaratAdministrasi(seminarId);
  if (!syaratData) {
...
async function submitVerifikasi(): Promise<void> {
  ...
  try {
    const json = await apiFetch<VerifySyaratResponse>(
      `/auth/seminar/${seminarId}/syarat-administrasi/verify`,
      {
        method: "PATCH",
        body: JSON.stringify({
          status,
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    // Load data seminar dan daftar dosen secara paralel
    await Promise.all([loadSeminar(seminarId), loadDosenOptions()]);
