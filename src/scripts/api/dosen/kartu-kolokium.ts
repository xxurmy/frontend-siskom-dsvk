...
// GET  /auth/kartu-kolokium/my                 -> daftar kartu kolokium yang dimoderatori dosen ini
...
async function loadKartuKolokium(): Promise<void> {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/auth/kartu-kolokium/my`, {
      headers: {
    ...
    const data: KartuKolokiumListResponse = await res.json();
    renderTable(data.kartu_kolokiums ?? []);
  } catch (err) {
